...
using UnityEngine;
using SecondOrderEasing;

[ExecuteAlways]
public class OLD_SecondOrderTransform : MonoBehaviour
{
    ...
    OLD_SecondOrder_1D oneDimensionDyna;
    OLD_SecondOrder_2D twoDimensionDyna;
    OLD_SecondOrder_3D threeDimensionDyna;
    OLD_SecondOrder_Quaternion quaternionDimensionDyna;


    // And we create a TransformData to hold all of the old transform components of our target.
    OLD_TransformData oldFollowTransform;
    ...
    private OLD_SecondOrder_1D GetDyna1D()
    {
        Vector4 followedValues = GetTargetValues(followTransform);

        // If our dynamics have not been initialized.
        if (oneDimensionDyna == null)
            oneDimensionDyna = new OLD_SecondOrder_1D(frequency, damping, initialResponse, followedValues.x);
    ...
    private OLD_SecondOrder_2D GetDyna2D()
    {
        Vector4 followedValues = GetTargetValues(followTransform);
        ...
        {
            twoDimensionDyna = new OLD_SecondOrder_2D(frequency, damping, initialResponse, 
    ...
    private OLD_SecondOrder_3D GetDyna3D()
    {
        Vector4 followedValues = GetTargetValues(followTransform);
        ...
        {
            threeDimensionDyna = new OLD_SecondOrder_3D(frequency, damping, initialResponse, 
    ...
    private OLD_SecondOrder_Quaternion GetQuaternionDyna()
    {
        Vector4 followedValues = GetTargetValues(followTransform);
        ...
        {
            quaternionDimensionDyna = new OLD_SecondOrder_Quaternion(frequency, damping, initialResponse,
    ...
    private OLD_TransformData OldTransform()
    {
        // If our oldFollowTransform isn't initialized, we create a new one.
        //if (oldFollowTransform == null)
            oldFollowTransform = new OLD_TransformData(followTransform);
    ...
    private Vector4 GetTargetValues(OLD_TransformData t)
    {
        List<float> results = new List<float>();
