...
public class OLD_SecondOrder_Quaternion
// Taken from the following video by t3ssel8r: https://www.youtube.com/watch?v=KPoeNZZ6H4s
{
    ...
    private OLD_SecondOrder_1D xValue, yValue, zValue, wValue;
    bool[] whichRotationToFollow = new bool[3];


    public OLD_SecondOrder_Quaternion(float f, float z, float r, Quaternion originalQuaternion)
    {
        // Initializing our 4 SecondOrder_1D values.
        xValue = new OLD_SecondOrder_1D(f, z, r, originalQuaternion.x);
        yValue = new OLD_SecondOrder_1D(f, z, r, originalQuaternion.y);
        zValue = new OLD_SecondOrder_1D(f, z, r, originalQuaternion.z);
        wValue = new OLD_SecondOrder_1D(f, z, r, originalQuaternion.w);
