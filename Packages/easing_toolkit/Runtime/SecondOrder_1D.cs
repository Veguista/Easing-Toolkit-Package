...
namespace SecondOrderDynamics
{
    ...
    public struct SecondOrder_1D
    {
        ...
        public float Update(float deltaTime, float targetValue, float inputVelocity)
        {
            // Error catching.
            if (deltaTime <= 0)
            {
            ...
            _lastSmoothedValue = _lastSmoothedValue + deltaTime * _yd; // Integrate position by inputVelocity.
            _yd = _yd + deltaTime * (targetValue + _myConstants.k3 * inputVelocity - _lastSmoothedValue - k1_stable * _yd)
                / k2_stable; // Integrate inputVelocity by acceleration.
        ...
        public float Update(float deltaTime, float targetValue)
        {
            ...
            // Estimate inputVelocity of change of the target value.
            float inputVelocity = (targetValue - _storedUnsmoothedTargetValue) / deltaTime;
